...
function get_items(){
	...
	if((int)$_REQUEST['id_loaicanho']!='')

	{

		$where.=" and id_loaicanho=".(int)$_REQUEST['id_loaicanho']."";
...

	{
		...
		$where.="and tenkhongdau like '%".$_REQUEST['key']."%' or ten like '%".$_REQUEST['key']."%' or masp like '%".$_REQUEST['key']."%'";

	}
	if((int)$_REQUEST['number']!='')

	{

		$where.=" and id ='".$_REQUEST['number']."'";
...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", _format_duoihinh, _upload_sanpham,$file_name)){
			...
			$data['thumb'] = create_thumb($data['photo'], 510 , 307, _upload_sanpham,$file_name,1);									
		...
		$data['id_loaicanho'] = (int)$_POST['id_loaicanho'];

		$data['id_loaitienich'] = implode(',',$_POST['id_loaitienich']);
		...
		$data['sophongngu'] = (int)$_POST['sophongngu'];
		...
		$data['soldout'] = isset($_POST['soldout']) ? 1 : 0;
		...
		$data['tenchunha'] = $_POST['tenchunha'];

		$data['sdtchunha'] = $_POST['sdtchunha']; 

		$data['phuong'] = $_POST['phuong']; 

		$data['quan'] = $_POST['quan'];  

		$data['namtrenduong'] = $_POST['namtrenduong']; 

		$data['sonha'] = $_POST['sonha'];  
		$data['nhomkhohang'] = $_POST['nhomkhohang'];  


		$data['giadien'] = $_POST['giadien'];  
		$data['gianuoc'] = $_POST['gianuoc'];  
		$data['phidichvu'] = $_POST['phidichvu'];  
		$data['phiguixe'] = $_POST['phiguixe'];  
		$data['phihoahongchutro'] = $_POST['phihoahongchutro'];  
				...
				for($i=0;$i<count($_FILES['files']['name']);$i++){
					...
					if($_FILES['files']['name'][$i]!=''){
...

						{
							...
							$data1['thumb'] = create_thumb($data1['photo'], 510 , 307, _upload_hinhthem,$file_name,1);	
