...
{				
	...
	window.location ="index.php?com=product&act=man&type=<?=$_REQUEST['type']?>&id_danhmuc="+a.value+"&id_loaicanho="+b.value;	
	return true;
}
...
{	
	var a = $('input.key').val();
	let b=removeVietnameseTones(a.toLowerCase());
	const arrName=b.split(" ");
	let result=arrName[0];
	...
	if(a=='Tên...') a='';		
	window.location ="index.php?com=product&act=man&type=<?=$_REQUEST['type']?>&key="+result;	
	return true;
}
function timkiemsothutu()
{	
	var a = $('input.number').val();
	console.log(a)	
	window.location ="index.php?com=product&act=man&type=<?=$_REQUEST['type']?>&number="+a;	
	return true;
}	
</script>
...
function get_main_list()
{
	...
	while ($row=@mysql_fetch_array($stmt)) 
	{
		if($row["id"]==(int)@$_REQUEST["id_loaicanho"])
			$selected="selected";
		else 
			$selected="";
		$str.='<option value='.$row["id"].' '.$selected.'>'.$row["ten"].'</option>';			
...
<form name="frm" id="frm" method="post" action="index.php?com=product&act=savestt<?php if($_REQUEST['id_danhmuc']!='') echo'&id_danhmuc='.$_REQUEST['id_danhmuc'];?><?php if($_REQUEST['id_list']!='') echo'&id_list='.$_REQUEST['id_list'];?><?php if($_REQUEST['id_cat']!='') echo'&id_cat='.$_REQUEST['id_cat'];?><?php if($_REQUEST['id_item']!='') echo'&id_item='.$_REQUEST['id_item'];?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>">
	...
	<div class="widget">
		...
		<div class="timkiem">
			<input type="text" value="" name="number" class="number"  placeholder="Nhập thứ tự tìm kiếm ">
			<button type="button" class="blueB" onclick="timkiemsothutu();" value="">Tìm kiếm qua thư tự</button>
		</div>
	</div>
	...
	<table cellpadding="0" cellspacing="0" width="100%" class="sTable withCheck mTable" id="checkAll">
		<thead>
			<tr>
				...
				<td class="tb_data_small">Sold Out</td>
				<td class="tb_data_small none">Khuyến mãi</td>
				<td class="tb_data_small none">Tiêu biểu</td>
		...
		<tbody>
			<?php for($i=0, $count=count($items); $i<$count; $i++){?>
				<tr>
					...
					<td align="center" >
						<?php
						$sql = "select ten from table_news_danhmuc where id='".$items[$i]['id_loaicanho']."'";
					...
					<td align="center" class="none">
						...
						<td align="center" >
							<a data-val2="table_<?=$_GET['com']?>" rel="<?=$items[$i]['soldout']?>" data-val3="soldout" class="diamondToggle <?=($items[$i]['soldout']==1)?"diamondToggleOff":""?>" data-val0="<?=$items[$i]['id']?>" ></a> 
						</td>
