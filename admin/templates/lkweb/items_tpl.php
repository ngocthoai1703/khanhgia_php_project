<script language="javascript" type="text/javascript">
	...
	$("#xoahet").click(function(){
		...
		if (hoi==true) document.location = "index.php?com=lkweb&act=delete&type=<?=$_REQUEST['type']?>&listid=" + listid;
	});
	});
	...
	{	
		...
		window.location ="index.php?com=lkweb&act=man&type=<?=$_REQUEST['type']?>&key="+a;	
		return true;
	}
</script>
...
<div class="control_frm" style="margin-top:25px;">
    <div class="bc">
        <ul id="breadcrumbs" class="breadcrumbs">
        	<li><a href="index.php?com=lkweb&act=man&type=<?=$_REQUEST['type']?>"><span>Liên kết web</span></a></li>
        	<?php if($_GET['key']!=''){ ?>
...
<div class="control_frm" style="margin-top:0;">
  	<div style="float:left;">
    	<input type="button" class="blueB" value="Thêm" onclick="location.href='index.php?com=lkweb&act=add&type=<?=$_REQUEST['type']?>'" />
     ...
     <?php for($i=0, $count=count($items); $i<$count; $i++){?>
        ...
        <td>
         ...
         <td class="title_name_data">
            <a href="index.php?com=lkweb&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?>" class="tipS SC_bold"><?=$items[$i]['link']?></a>
        </td>
