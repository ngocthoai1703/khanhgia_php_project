...
<form name="supplier" id="validate" class="form" action="index.php?com=news&act=save_danhmuc<?php if($_REQUEST['p']!='') echo'&p='.$_REQUEST['p'];?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>" method="post" enctype="multipart/form-data">
     ...
     <div class="widget">
       ...
       <div id="info" class="tab_content">
        ...
        <?php if($_GET['type']=='sphongngu' || $_GET['type']=='sdientich' || $_GET['type']=='sgiamax' || $_GET['type']=='sgiamin'){ ?>
          <div class="formRow">
            <label>Giá trị</label>
            <div class="formRight">
                <input type="text" value="<?=@$item['noidung']?>" name="noidung" title="Nhập giá trị" class="tipS" />
            </div>
            <div class="clear"></div>
        </div>
